...
2. Add the serde_json crate to Cargo.toml as a dependency for reading and writing JSON
...
5. Define a constant for the JSON file path that will store the to-do list
6. Implement a function to read and parse the JSON file into a Vec of Task structs
7. Implement a function to write a Vec of Task structs back to the JSON file
8. Implement CLI argument parsing to extract the operation and parameters from command-line arguments
9. Implement the list operation to read the JSON, display each task with title and status, and show an empty message if the list is empty
10. Implement the add operation to create a new Task and append it to the JSON array
...
13. Implement the delete operation to remove an existing task from the JSON array by index
